...
pathin2=[path,'outputVS1\WakeEnhance\'];%pathF=[path,'Figures\WakeEnhance']; mkdir(pathF)
...
SPafter=[];
Corfs=[];
for geno=1:2 %3 %1:GFP 2:ChR2
    if geno==1
        mousenames=mousenames1; Base=Base1; SDo=SDonly1; SDs=SDstim1; genoN=genoNs(geno,:); pathin=pathin0; colorsp=[0, 0, 255]./255;
    elseif geno==2
        mousenames=mousenames2; Base=Base2; SDo=SDonly2; SDs=SDstim2; genoN=genoNs(geno,:); pathin=pathin1; colorsp=[255, 0, 0]./255;
    elseif geno==3
    ...
    SPo_score2=[];
    SPn_score2=[];
    Wmat1=[]; Nmat1=[]; Rmat1=[]; Smat1=[]; 
    Wmat2=[]; Nmat2=[]; Rmat2=[]; Smat2=[]; 
    ...
    for n=1:numanim
        ...
        eval(['load ',pathin2,fn2,'.mat Wso Nso Rso Sso -mat']);
        Wmat1=[Wmat1; Wso];
        Nmat1=[Nmat1; Nso];
        Rmat1=[Rmat1; Rso];
        Smat1=[Smat1; Sso];
...
%         SPn_BfAf=[SPn_BfAf;sp4./sp3*100];
    ...
    incrSWA=SPaSD./SPbSD;
    SWAr=mean(incrSWA(:,3:17),2);
    
    tWr=Wmat2-Wmat1; tWr=tWr./15; %tNRr=Nmat2-Nmat1; tNRr=tNRr./15;

    figure (5)
    subplot(2,2,1)
    scatter(tWr(:,1), SWAr, [], colorsp)
    hold on
    AA=[tWr(:,1), SWAr];
    axis square
    [R1,P1,RL1,RU1] = corrcoef(AA,'Alpha',0.01);
    currSlope = polyfit(tWr(:,1),SWAr,1);
    xlims = get(gca,'XLim');
    regLine = currSlope(2) + (xlims(1):xlims(2)).*currSlope(1);
    plot(xlims,regLine([1,end]),'-','Color' ,colorsp)
    
    
    hold on
    subplot(2,2,2)
    scatter(tWr(:,2), SWAr, [], colorsp)
    hold on
    AA=[tWr(:,2), SWAr];
    axis square
    [R2,P2,RL2,RU2] = corrcoef(AA,'Alpha',0.01);
    currSlope = polyfit(tWr(:,2),SWAr,1);
    xlims = get(gca,'XLim');
    regLine = currSlope(2) + (xlims(1):xlims(2)).*currSlope(1);
    plot(xlims,regLine([1,end]),'-','Color' ,colorsp)
    
    saveSWAtWr=[SWAr tWr];
    Corfs=[Corfs; R1(2,1) P1(2,1) R2(2,1) P2(2,1)];
...
%     
    ...
    if geno==1; 
        aPrismgfp1=SPo_score'; aPrismgfp2=SPn_score'; aPrismgfpdiff=(SPn_score-SPo_score)'; 
        aPrismgfpbeforeafter=(100*SPaSD./SPbSD)'; aPrism_SWAtWr_GFP=saveSWAtWr';
    elseif geno==2; 
        aPrismchr1=SPo_score'; aPrismchr2=SPn_score';  aPrismchrdiffA=(SPn_score-SPo_score)'; 
        aPrismchr1beforeafter=(100*SPaSD./SPbSD)'; aPrism_SWAtWr_LPO=saveSWAtWr';
    elseif geno==3; 
        aPrismchr3=SPo_score'; aPrismchr4=SPn_score';  aPrismchrdiffB=(SPn_score-SPo_score)'; 
        aPrismchr2beforeafter=(100*SPaSD./SPbSD)'; aPrism_SWAtWr_nonLPO=saveSWAtWr';
    end
