...
% 
% mousenames2=[1 5 6 7 8 9 12 16 17 18 19 20 21]; 
% days2=['010518';'010518';'050518';'150518';'220618';'220618';'220618';'150119';'150119';'220819';'220819';'220819';'220819']; 
% injtime=[];
% firststims2=[600;600;755;537;810;810;758;758;1125;1125;1125;1125];
% stimends2=[7769;7769;7854;8515;8178;8178;4064;4064;4530;4530;4530;4530]; %%%%if >3430+firststim, >16times stim
...
after=2;   %2min of stim, and the 2min after 
...
else %geno=4 looks at the exp group
...
%     tfs=firststims(anim,:);
    ...
    Ws=[]; Ds=[]; Ms=[]; LmatD=[]; LmatW=[];%creates empty arrays to be filled 
    for s=1:numstim %iterating over all the stimulations
        ...
        Ws=[Ws;ws];  %for each of the 72 stims 
        LWmat=ws(1,31:60);
        ...
        ds=dex(eps);  %total sleep (nrem and rem)
        Ds=[Ds;ds];
        LDmat=ds(1,31:60);
        LD1=min(find(LDmat<1));
        ...
        Ls(numstim*(anim-1)+s,:)=[mousenames(anim) s LD1*4 LW1*4];

        
        LmatD=[LmatD LD1*4]; LmatW=[LmatW LW1*4];
...
% %     eval(['save ',pathout,fn4,'.mat tfs Ws Ms Ds -mat']);
   ...
   LmeanD=nanmean(LmatD); LmeanW=nanmean(LmatW);
   Lmeans=[Lmeans; mousenames(anim) LmeanD LmeanW];

end

aPrism_Latency=Lmeans'
aPrism_Latency_v2=Ls;
LatencyDex=[mousenames; DallMice(:,30:61)']
aPrism_LatencyDex=LatencyDex'
