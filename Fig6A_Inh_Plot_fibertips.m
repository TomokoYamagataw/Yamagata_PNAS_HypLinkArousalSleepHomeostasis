...
% mousenames=[1 5 6 7 8 9 12 15 16 17 18 19 20 21]; %ChR2
mousenames=[1 2 4 5 8 9]; %Arch
numanim=length(mousenames);


% [AP, ML, DV from Bregma]

for nn = 1:numanim
%%%%% Arch
ftips=[ 0.10 0.70 -5.6;... %1 Arch1 slide 43,55,57/67 (AP0.26, AP0.14,#,AP0.02, AP-0.10)  low expression, injection site damaged
        0.05 0.70 -5.5;... %2 Arch2 slide 18-28/31 ((AP0.26), AP0.14,# AP0.02, AP-0.1, AP-0.22)  low expression
       -0.10 0.80 -5.6;... %4 Arch4 slide 68-75/168 (AP0.02, #AP-0.10, AP-0.22) low expression, Histology very tilted and damaged
       -0.15 0.70 -5.25;... %5 Arch5 slide 26-32 (AP0.02, AP-0.1,#, AP-0.22, AP-0.34)
       -0.50 0.60 -5.1;... %8 Arch8 slide 20,22 (AP-0.34, AP-0.46, #AP-0.58, AP-0.7)
       -0.05 0.65 -5.4;... %9 Arch9 slide 5-15/15 (AP0.14, #AP0.02, #AP-0.1,AP-0.22) Best
       ]; 
...
stem3(x2, y2, z2, 'filled','Color',[230/255 159/255 0]);
hold on
stem3(x3, y3, z3, 'filled','Color',[100/255 80/255 0/255]);
...
ax.FontSize = 10;
h2=scatter3(x2, y2, z2, cs,[230/255 159/255 0]);
hold on
h3=scatter3(x3, y3, z3, cs,[100/255 80/255 0/255]);
