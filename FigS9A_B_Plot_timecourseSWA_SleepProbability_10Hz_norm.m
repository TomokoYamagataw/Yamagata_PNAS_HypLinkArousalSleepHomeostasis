...
before=5; 
after=25;   %it gives me 5min before, 5min of stim, and the 20min after 
...
elseif geno==7
    mousenames=mousenames7; days=days7; gn='';
end
...
sleepp=reshape(sleep,[],bin); 
sleepp=nanmean(sleepp);
...
for pp=1 %1:bin %1
        ...
        for anim=1:numanim 
            ...
            daysi=days(anim,:); is=find(isspace(daysi)); 
                ...
                if geno<8;  outs=find((stimep2-stimep1)<70);stimep1(outs)=[];stimep2(outs)=[]; end
