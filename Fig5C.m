...
pathStim = [path,'STIMs\']; %pathStim = 'I:\optogenetics\STIMs';
...
% mousenames1=[1 2 3 4 6 8]; %GFP controls % GFP4: 040718 to 260618 % GFP5: 031018 to 051018
% days1=['250218 260218';'290418 120418';'300618 260618';'300618 040718';'081018 061018';'080119 070119'];
...
pathvs=[path,'outputVSall\'];% relic from old cold can be replaced
int=2;
numint=24/int;
...
for vs = 1:4
    ...
    statsTable.mouseID = repmat(1:19,1,2)'; %statsTable.mouseID = repmat(1:22,1,2)';
...
for geno = 1:numel(wSpectraOcc) %2:numel(wSpectraOcc) 
    nonEmptyCells = ~cellfun(@isempty,wSpectraFro{geno}); 
    nonEmptyCells(:,:)=[nonEmptyCells(:,1).*nonEmptyCells(:,2) nonEmptyCells(:,1).*nonEmptyCells(:,2)]; %Tomoko added
...
toPlotLPO = toPlotALL(nMiceGFP+1:nMiceGFP+nMiceLPO,:); 
toPlotnLPO =toPlotALL(nMiceGFP+nMiceLPO+1:end,:); 
...
for geno = 1:numel(wSpectraOcc) %2:numel(wSpectraOcc) 
    nonEmptyCells = ~cellfun(@isempty,wSpectraFro{geno}); 
    nonEmptyCells(:,:)=[nonEmptyCells(:,1).*nonEmptyCells(:,2) nonEmptyCells(:,1).*nonEmptyCells(:,2)]; %Tomoko added
    stimbase=[stimbase; nonEmptyCells];
end
SWALightStim = cat(1,mean(rez(1).swa(:,:,1:6),3),mean(rez(2).swa(:,:,1:6),3),mean(rez(3).swa(:,:,1:6),3)); 
SWALightBl = cat(1,mean(rez(1).swa(:,:,13:18),3),mean(rez(2).swa(:,:,13:18),3),mean(rez(3).swa(:,:,13:18),3)); 
diffNRSWA = SWALightStim-SWALightBl;
diffNRSWAmod=diffNRSWA(stimbase(:,1)==1);
...
for pltCnt = 1:numel(toCorrelate)
    subplot(2,2,pltCnt)
%     xL = [-200 400];  line(xL, [0 0],'Color',[.8 .8 .8],'LineStyle',':');  %x-axis 
%     hold on
%     yL = [-1000 2000];  line([0 0], yL,'Color',[.8 .8 .8],'LineStyle',':');  %y-axis
    AA1=[toCorrelate{pltCnt}, diffNRSWAmod];
    [R1,P1,RL1,RU1] = corrcoef(AA1,'Alpha',0.05);
    currCorr=corrcoef(AA1); ylabel('\Delta NREM SWA');
    currSlope = polyfit(toCorrelate{pltCnt}, diffNRSWAmod,1);
       
    corrCorrAllmat=[];corrfpmat=[];
    for ii=1:geno
        if ii==1; cType=1:nMiceGFP; elseif ii==2; cType=nMiceGFP+1:nMiceGFP+nMiceLPO; elseif ii==3; cType=(nMiceGFP+nMiceLPO+1:length(diffNRSWAmod)); end    
    scatter(toCorrelate{pltCnt}(cType), diffNRSWAmod(cType),15,'MarkerFaceColor',colorsToUse{ii},'MarkerEdgeColor','none');hold on;
    hold on
    AA2=[toCorrelate{pltCnt}(cType), diffNRSWAmod(cType)];
    [R2,P2,RL2,RU2] = corrcoef(AA2,'Alpha',0.05);
%     currCorr = corrcoef(AA2);
    currSlope = polyfit(toCorrelate{pltCnt}(cType), diffNRSWAmod(cType),1);
  
    xlims = get(gca,'XLim');
    regLine = currSlope(2) + (xlims(1):xlims(2)).*currSlope(1);
    plot(xlims,regLine([1,end]),'-','Color' ,colorsToUse{ii})
    corrCorrAllmat=[corrCorrAllmat R2(2)];
    corrfpmat=[corrfpmat  P2(2)];
    end
    corrCorrAll=[corrCorrAll; corrCorrAllmat];   
    corrP=[corrP; corrfpmat];   

    xlabel(xLabelling{pltCnt});
    title(['R=',num2str(corrCorrAllmat(2)) ' p=',num2str(corrP(2))]);
    
    xlim(get(gca,'XLim')+([-50 50])); % xlim([-160 160]); %
    ylim([-400 800]);%ylim(get(gca,'YLim')+([-50 50])); 
    xL = xlim; line(xL, [0 0],'Color',[.8 .8 .8],'LineStyle',':');  %x-axis 
    yL = ylim; line([0 0], yL,'Color',[.8 .8 .8],'LineStyle',':');  %y-axis
    set(gca,'TickDir','out','FontName','Arial','FontSize',9)
    
end
corrCorrAll
corrP
saveas(gcf,fullfile(pathFigs,'corrSWAThetaR4.fig'));
saveas(gcf,fullfile(pathFigs,'corrSWAThetaR4.png'));
saveas(gcf,fullfile(pathFigs,'corrSWAThetaR4.svg'));



% stats: compare correlation coefficients (Andy Field pp 362), for dependent corrcoefs 
% Weaver, B., & Wuensch, K. L. (2013). SPSS and SAS programs for comparing Pearson correlations and OLS regression coefficients. Behavior research methods, 45(3), 880-895: p. 892
% Meng X-L, Rosenthal R, Rubin DB. Comparing correlated correlation coefficients. Psychological bulletin 1992;111(1):172-175.

Rxy = corr([diffWThetaOcc(7:13),diffNRSWAmod(7:13)]);
Rzy = corr([diffWSWAFro(7:13),diffNRSWAmod(7:13)]);
Rxz = corr([diffWThetaOcc(7:13),diffWSWAFro(7:13)]);
Rxy = Rxy(2); Rzy = Rzy(2); Rxz = Rxz(2); 
nMice = length(diffWThetaOcc(7:13));
tValLPO = (Rxy - Rzy).*sqrt((nMice-3).*(1+Rxy)./(2.*(1-Rxy^2-Rxz^2-Rzy^2+2*Rxy*Rxz*Rzy)))
tinv(0.999,nMice-3)
tinv(0.99,nMice-3)
tinv(0.95,nMice-3)

Rxy = corr([diffWThetaOcc(14:end),diffNRSWAmod(14:end)]);
Rzy = corr([diffWSWAFro(14:end),diffNRSWAmod(14:end)]);
Rxz = corr([diffWThetaOcc(14:end),diffWSWAFro(14:end)]);
Rxy = Rxy(2); Rzy = Rzy(2); Rxz = Rxz(2); 
nMice = length(diffWThetaOcc(14:end));
tValnLPO = (Rxy - Rzy).*sqrt((nMice-3).*(1+Rxy)./(2.*(1-Rxy^2-Rxz^2-Rzy^2+2*Rxy*Rxz*Rzy)))
tinv(0.999,nMice-3)
tinv(0.99,nMice-3)
tinv(0.95,nMice-3)

Rxy = corr([diffWThetaOcc(7:end),diffNRSWAmod(7:end)]);
Rzy = corr([diffWSWAFro(7:end),diffNRSWAmod(7:end)]);
Rxz = corr([diffWThetaOcc(7:end),diffWSWAFro(7:end)]);
Rxy = Rxy(2); Rzy = Rzy(2); Rxz = Rxz(2); 
nMice = length(diffWThetaOcc(7:end));
tValAll = (Rxy - Rzy).*sqrt((nMice-3).*(1+Rxy)./(2.*(1-Rxy^2-Rxz^2-Rzy^2+2*Rxy*Rxz*Rzy)))
tinv(0.999,nMice-3)
tinv(0.99,nMice-3)
tinv(0.95,nMice-3)


%%%%% stats: z - for dependent corrcoefs

matLPO=7:13; matnonLPO=14:19; nL=length(matLPO); nN=length(matnonLPO);
rLPO=corr([diffWThetaOcc(7:13),diffNRSWAmod(7:13)]); rLPO=rLPO(2)
%%%% z transformation: Fisher,R.A 1950 Statistical Methods research workers.
zLPO=1.82
rnonLPO=corr([diffWThetaOcc(14:end),diffNRSWAmod(14:end)]); rnonLPO=rnonLPO(2)
znonLPO=0.445
z_wOcctheta=(zLPO-znonLPO)/sqrt(1/(nL-3)+1/(nN-3))
%%%% if <
...
minMax = [-3 3.2]; %minMax = [min(minMax(:)) max(minMax(:))];
for plotCnt = 1:2
    subplot(2,2,plotCnt)
